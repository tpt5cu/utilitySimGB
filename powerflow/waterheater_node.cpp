/** $Id: waterheater_node.cpp 1186 2017-01-02 18:15:30Z jingkun $
	Copyright (C) 2017 Carnegie Mellon University
	@file waterheater_node.cpp
	@addtogroup waterheater_node
...
CLASS* waterheater_node::oclass = NULL;
CLASS* waterheater_node::pclass = NULL;

waterheater_node::waterheater_node(MODULE *mod) : node(mod)
{
	if(oclass == NULL)
	{
		pclass = node::oclass;
		
		oclass = gl_register_class(mod,"waterheater_node",sizeof(waterheater_node),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==NULL)
			throw "unable to register class waterheater_node";
		else
		...
		if(gl_publish_variable(oclass,
			...
			if (gl_publish_function(oclass,	"delta_linkage_node", (FUNCTIONADDR)delta_linkage)==NULL)
				GL_THROW("Unable to publish waterheater_node delta_linkage function");
			if (gl_publish_function(oclass,	"interupdate_pwr_object", (FUNCTIONADDR)interupdate_waterheater_node)==NULL)
				GL_THROW("Unable to publish waterheater_node deltamode function");
			if (gl_publish_function(oclass,	"delta_freq_pwr_object", (FUNCTIONADDR)delta_frequency_node)==NULL)
				GL_THROW("Unable to publish waterheater_node deltamode function");
    }
}

int waterheater_node::isa(char *classname)
{
	return strcmp(classname,"waterheater_node")==0 || node::isa(classname);
}

int waterheater_node::create(void)
{
...
int waterheater_node::init(OBJECT *parent)
{
	if ( !(has_phase(PHASE_S)) )
	{
		OBJECT *obj = OBJECTHDR(this);
		gl_warning("Init() waterheater_node (name:%s, id:%d): Phases specified did not include phase S. Adding phase S.", obj->name,obj->id);
		/* TROUBLESHOOT
		waterheater nodes and meters require a single phase and a phase S component (for split-phase).
		This particular waterheater object did not include it, so it is being added.
		*/
...
//Functionalized waterheater_node presync -- mainly so 
//External deltamode calls still execute things right
void waterheater_node::BOTH_waterheater_node_presync_fxn(void)
{
	//Clear the shunt values
	shunt[0] = shunt[1] = shunt[2] = 0.0;
}

TIMESTAMP waterheater_node::presync(TIMESTAMP t0)
{
	//Call the functionalized version
	BOTH_waterheater_node_presync_fxn();
...
//For external calls
void waterheater_node::BOTH_waterheater_node_sync_fxn(void)
{
...
TIMESTAMP waterheater_node::sync(TIMESTAMP t0)
{
	//Call the functionalized version
	BOTH_waterheater_node_sync_fxn();
...
//////////////////////////////////////////////////////////////////////////
//Module-level call
SIMULATIONMODE waterheater_node::inter_deltaupdate_waterheater_node(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val,bool interupdate_pos)
{
	...
	{
		//Call waterheater-specific call
		BOTH_waterheater_node_presync_fxn();

		//Call node presync-equivalent items
		NR_node_presync_fxn(0);

		//Call sync-equivalent of waterheater portion first
		BOTH_waterheater_node_sync_fxn();
...
*/
EXPORT int create_waterheater_node(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(waterheater_node::oclass);
		if (*obj!=NULL)
		{
			waterheater_node *my = OBJECTDATA(*obj,waterheater_node);
	...
	CREATE_CATCHALL(waterheater_node);
}
...
*/
EXPORT int init_waterheater_node(OBJECT *obj)
{
	try {
		waterheater_node *my = OBJECTDATA(obj,waterheater_node);
		return my->init();
	}
	INIT_CATCHALL(waterheater_node);
}
...
*/
EXPORT TIMESTAMP sync_waterheater_node(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	try {
		waterheater_node *pObj = OBJECTDATA(obj,waterheater_node);
		TIMESTAMP t1;
		switch (pass) {
		case PC_PRETOPDOWN:
	...
	SYNC_CATCHALL(waterheater_node);
}

EXPORT int isa_waterheater_node(OBJECT *obj, char *classname)
{
	return OBJECTDATA(obj,waterheater_node)->isa(classname);
}

EXPORT int notify_waterheater_node(OBJECT *obj, int update_mode, PROPERTY *prop, char *value){
	waterheater_node *n = OBJECTDATA(obj, waterheater_node);
	int rv = 1;
...
//Deltamode export
EXPORT SIMULATIONMODE interupdate_waterheater_node(OBJECT *obj, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val, bool interupdate_pos)
{
	waterheater_node *my = OBJECTDATA(obj,waterheater_node);
	SIMULATIONMODE status = SM_ERROR;
	try
	{
		status = my->inter_deltaupdate_waterheater_node(delta_time,dt,iteration_count_val,interupdate_pos);
		return status;
	}
	catch (char *msg)
	{
		gl_error("interupdate_waterheater_node(obj=%d;%s): %s", obj->id, obj->name?obj->name:"unnamed", msg);
		return status;
	}
}
